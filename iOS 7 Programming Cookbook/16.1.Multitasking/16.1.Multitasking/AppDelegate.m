...
- (void) timerMethod:(NSTimer *)sender {
    ...
    NSLog(@"%@", (bgTimeRemaining == DBL_MAX)?@"Undetermined":[NSString stringWithFormat:@"%.02f s", bgTimeRemaining]);
}
...
- (void)endBackgroundTask {
    ...
    dispatch_async(mainQueue, ^{
        ...
        if (strongSelf != nil) {
            ...
            strongSelf.bgTaskIdentifier = UIBackgroundTaskInvalid;
        }
    });
}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    NSLog([self isMultitaskingSupported]?@"yes":@"no");
    return YES;
}
