...
- (NSFetchRequest *)newFetchRequest {
    NSFetchRequest *req = [[NSFetchRequest alloc] initWithEntityName:NSStringFromClass([Person class])];
    
    req.fetchBatchSize = 20;
    req.predicate = [NSPredicate predicateWithFormat:@"(age >= -1000) AND (age <= 2000)"];
    req.resultType = NSManagedObjectIDResultType;
...
- (void) processPersons:(NSArray *)persons {
    ...
    for (Person *p in persons) {
        NSLog(@"%d, %@, %@, %d", ++i, p.firstName, p.lastName, p.age.intValue);
    }
}
...
//    [self populateDatabase];
    ...
    [bgContext performBlock:^{
        NSError *err = nil;
        NSArray *personIds = [bgContext executeFetchRequest:[weakSelf newFetchRequest] error:&err];
        
        if (personIds != nil && err == nil) {
            mutablePersons = [[NSMutableArray alloc] initWithCapacity:personIds.count];
            
            
            dispatch_async(dispatch_get_main_queue(), ^{
                for (NSManagedObjectID *personId in personIds) {
                    Person *p = (Person *)[mainContext objectWithID:personId];
                    [mutablePersons addObject:p];
                }
...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"_8_9_Fetching_Data_in_BG" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"_8_9_Fetching_Data_in_BG.sqlite"];
